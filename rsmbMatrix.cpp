/*
 * Function name  : rsmbMatrix.cpp
 * Author name    : Vijay Rengarajan
 * Creation date  : June 9, 2015
 *
 * Syntax:
 * targetMatrix = rsmbMatrix(sourceSize, homographies, weights, nhomos, sourceOrigin)
 *
 ...
 *   Every homography in the set of homographies (for each row) corresponds
 ...
 *   created. Note that each row can have different number of homographies. 
 *   We take the maximum count from all rows, and create number of columns
 *   according to this number. For other pixels which have less than this 
 *   maximum count, indices and weights are filled with zeros. If the 
 *   source indices map outside the size of the image, we map zero.
 * 
 * Updates:
 * June 9, 2015 : Created
 */
 ...
 * the user.
 */
void rsmbMatrix(double *target, double *origH, double *x, double *nhomos, 
        double max_nhomos, int nrows, int ncols, double orig_row, double orig_col)
{
    // Number of pixels
    int npix;
                ...
                for (ii=0; ii<2; ii++) {
                    for (jj=0; jj<2; jj++) {
                        ...
                        if (src_row >=0 && src_row < nrows && src_col >=0 && src_col < ncols) {    
                            target[(8*m + ii*2 + jj)*npix + tgt_idx] = src_idx;
                            target[(8*m + 4 + ii*2 + jj)*npix + tgt_idx] = thisx * wt;
                        }
                        else {
                            target[(8*m + ii*2 + jj)*npix + tgt_idx] = 0;
                            target[(8*m + 4 + ii*2 + jj)*npix + tgt_idx] = 0;
                        }
                    } // end for jj
                } // end for ii
            }
            /* Make the remaining set of columns to zero */
            if (nhomos[j] < max_nhomos) {
                for(m=(int)nhomos[j]; m<max_nhomos; m++) {
                    for (ii=0; ii<2; ii++) {
                        for (jj=0; jj<2; jj++) {
                            target[(8*m + ii*2 + jj)*npix + tgt_idx] = 0;
                            target[(8*m + 4 + ii*2 + jj)*npix + tgt_idx] = 0;
    ...
    delete[] wt_list;
    delete[] nhomos_cum;
//     delete[] thisH; // thisH points inside H, so no need of deallocating
    delete[] H;
}
...
{
    int nrows, ncols, ndim, j;
    
    double *target, *H,  *wt, *orig, *nhomos, *siz;
    double max_nhomos;
    
    /* Create pointer to inputs */
    siz = mxGetPr(prhs[0]); // size of source and target
    ...
    /* Get the max number of homographies used by the rows */
    max_nhomos = nhomos[0];
    for(j=1; j<nrows; j++) {
       if(nhomos[j] > max_nhomos) { max_nhomos = nhomos[j]; }
    }
    
    /* Create the output matrix */
    plhs[0] = mxCreateDoubleMatrix(nrows*ncols,8*max_nhomos,mxREAL);
    
    /* Get a pointer to the output matrix */
    target = mxGetPr(plhs[0]);
    
    /* Call the function */
    rsmbMatrix(target, H, wt, nhomos, max_nhomos, nrows, ncols, orig[0], orig[1]);
}